...
import psych.Alphabetp;
private var curOption:GameplayOption = null;
private var curSelGC:Int = 0;
...
function create() {
	...
	for (i in 0...optionsArray.length) {
		...
		if(optionsArray[i].type == 'bool') {
			...
			var valueText = new AttachedText(Std.string(optionsArray[i].getValue()), optionText.width-70, -10, true, 0.8);
...
function update(elapsed:Float) {
	...
	if(nextAccept <= 0) {
		...
		if(usesCheckbox) {
			...
			if(controls.LEFT || controls.RIGHT) {
				...
				if(holdTime > 0.5 || pressed) {
					if(pressed) {
						...
						if(curOption.type != 'string') add = controls.LEFT ? -curOption.changeValue : curOption.changeValue;
						...
						switch(curOption.type) {
							...
							case 'string':var num:Int = curOption.curOption; //lol
						...
						switch(curOption.type) {
							...
							case 'float'|'percent':var blah:Float = Math.max(curOption.minValue, Math.min(curOption.maxValue, holdValue + curOption.changeValue - (holdValue % curOption.changeValue)));
			...
			} else if(controls.LEFT_R || controls.RIGHT_R) clearHold();
		}
...
function clearHold(){if(holdTime > 0.5) FlxG.sound.play(Paths.sound('scrollMenu')); holdTime = 0;}

function changeSelection(a:Int = 0) {
	curSelGC = FlxMath.wrap(curSelGC+a,0,optionsArray.length-1);

	var bullShit:Int = 0;

	for (item in grpOptions.members) {
		item.targetY = bullShit - curSelGC;
		bullShit++;
	...
	for (text in grpTexts) {
		text.alpha = 0.6;
		if(text.ID == curSelGC) text.alpha = 1;
	}
	curOption = optionsArray[curSelGC]; //shorter lol
	FlxG.sound.play(Paths.sound('scrollMenu'));
}
...
class GameplayOption
{
	...
	private function get_type()
	{
		...
		switch(StringTools.trim(type.toLowerCase()))
		{
