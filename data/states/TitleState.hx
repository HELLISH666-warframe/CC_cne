...
var darkLord = new FlxSprite((FlxG.width/2),800).loadGraphic(Paths.image('menus/titlescreen/darkLord'));
...
function update(elapsed:Float) {
	...
	for (i in 0...socialMedia.length) {
		...
		if(i == 1 && FlxG.keys.justPressed.SHIFT) {//Twitter/X (ew X)
	...
	if(FlxG.mouse.overlaps(optionShortCut) && FlxG.mouse.justPressed) {
	...
	if(!transitioning && !skippedIntro) {
		...
		switch (sickBeats) {
			...
			case 25:
				...
				FlxTween.tween(logoBl, {x: 166}, 2, {type: FlxTween.ONESHOT, ease: FlxEase.backInOut});
			case 28:
				doNotZoom = true;
				zoomLerpTo = 0.7;
				zoomPerSec = 3;
			...
			case 30:FlxG.cameras.fade(FlxColor.WHITE, 1, false);
			case 33:
				FlxG.cameras.fade(FlxColor.WHITE, 0, true);
				skipIntro();
				zoomLerpTo = 1;
				zoomPerSec = 1000;
				FlxTween.tween(darkLord, {y: 0}, 1, {type: FlxTween.ONESHOT, ease: FlxEase.backInOut, startDelay: 0.5});
				FlxTween.tween(chosenOne, {y: 0}, 1, {type: FlxTween.ONESHOT, ease: FlxEase.backInOut, startDelay: 0.5});
		}
	}
}
...
function checkIfClicked(object:FlxSprite, id:Int) {//the tag is the thing used for the select void
	...
	switch(id) {
		case 0: CoolUtil.openURL('https://gamebanana.com/mods/599759');
