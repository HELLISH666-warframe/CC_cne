...
var portrait:FlxSprite = new FlxSprite();
 ...
 var chris_pratt:FlxText = new FlxText(500, 650, 500, "so_cool",44);
function shadering() 
{
...
function onChangeSelection(event) {
	...
	FlxTween.tween(portrait, {x: portrait.x + 45}, 0.2, {ease: FlxEase.quintIn, onComplete: function(twn:FlxTween) {
		...
		chris_pratt.text=""+songs[curSelected + val].displayName;
		zoomTween = FlxTween.tween(chris_pratt, {"scale.x": 0.85, "scale.y": 0.85}, 0.2, {
					ease: FlxEase.quadOut,
					onComplete: function(twn:FlxTween) {
						zoomTween = null;
					}
				});
		/*zoomTween = FlxTween.tween(chris_pratt, {"scale.x": 0.8, "scale.y": 1}, 0.2, {
					ease: FlxEase.quadOut,
					onComplete: function(twn:FlxTween) {
						zoomTween = null;
					}
