...
function startIntro() {
	...
	add(bg = new FlxSprite().loadGraphic(Paths.image('menus/title/background'))).antialiasing = Options.antialiasing;
	bg.alpha = 0.00001;

	bg2 = CoolUtil.loadAnimatedGraphic(new FlxSprite(), Paths.image('menus/title/background2'),60);//this shit uses like 230MB and I am not even joking
	...
	vignette = new FlxSprite().loadGraphic(Paths.image('menus/title/vignetteThings'));
	vignette.alpha = 0.00001;
	add(vignette);

	chosenOne = new FlxSprite(0, 800).loadGraphic(Paths.image('menus/title/chosenOne'));
	chosenOne.alpha = 0.00001;
	chosenOne.antialiasing = Options.antialiasing;
	add(chosenOne);

	darkLord = new FlxSprite((FlxG.width / 2), 800).loadGraphic(Paths.image('menus/title/darkLord'));
	darkLord.alpha = 0.00001;
	darkLord.antialiasing = Options.antialiasing;
	add(darkLord);

	smite = CoolUtil.loadAnimatedGraphic(new FlxSprite(), Paths.image('menus/title/thing'));
	smite.antialiasing = Options.antialiasing;
	...
	add(optionShortCut).antialiasing = Options.antialiasing;
...
function beatHit() {
	...
	if(!closedState) {
		sickBeats++;
		switch (sickBeats) {
			case 1:FlxG.sound.playMusic(Paths.music('freakyMenu'), 0);
