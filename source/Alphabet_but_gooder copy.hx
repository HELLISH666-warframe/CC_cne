...
class Alphabet_but_gooder extends Alphabet
{
	...
	override function update(elapsed:Float)
	{
		...
		if (isCreditItem)
		{
			var lerpValX:Float = FlxMath.bound(elapsed * 0.6 * 60, 0, 1);
			var lerpValY:Float = FlxMath.bound(elapsed * 0.2 * 60, 0, 1);
			if(changeX)
	...
	public override function set_alpha(val:Float) {
		//super.set_alpha(val);
		for (letter in letters)
			setFontColor(letter, fontColor);
		return alpha;
